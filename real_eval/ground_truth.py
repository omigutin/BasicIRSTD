...
def _prepare_negative_images(
    negative_roots: Sequence[Path],
) -> tuple[list[dict[str, str]], Mapping[str, int]]:
    """Валидирует negative-наборы до изменения любых итоговых CSV."""
...
def _write_negative_images(path: Path, rows: Sequence[Mapping[str, str]]) -> None:
    """Записывает предварительно проверенный negative-индекс."""
