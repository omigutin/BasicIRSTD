...
def find_coco_json(coco_export: Path) -> Path:
    ...
    if not coco_export.exists():
        raise FileNotFoundError(
            f"COCO export does not exist: {coco_export}"
        )

    if coco_export.is_file():
        candidates = (
            [coco_export]
            if coco_export.name == "_annotations.coco.json"
            else []
        )
    else:
        candidates = sorted(
            coco_export.rglob("_annotations.coco.json")
        )
...
def _require_list(
    ...
    if not isinstance(value, list):
        raise AuditError(
            f"COCO section '{section}' must be a list"
        )
...
def _load_coco(
    ...
    if not all(
        ...
        raise AuditError(
            "COCO images and annotations must contain objects"
        )
...
def _validate_unique_ids(
    ...
    if any(not isinstance(value, int) for value in ids):
        raise AuditError(
            f"Every COCO {name} record must have an integer id"
        )
    ...
    if duplicates:
        raise AuditError(
            f"Duplicate COCO {name} ids: {duplicates}"
        )
...
def _validate_annotations(
    ...
    for annotation in annotations:
        ...
        if "category_id" not in annotation:
            errors.append(
                f"annotation {annotation_id}: missing category_id"
            )

        if "segmentation" not in annotation:
            errors.append(
                f"annotation {annotation_id}: missing segmentation"
            )
        ...
        if (
            ...
            errors.append(
                f"annotation {annotation_id}: "
                "bbox is outside COCO image bounds"
            )
    ...
    if errors:
        raise AuditError(
            "COCO annotation audit failed:\n- "
            + "\n- ".join(errors)
        )

    return {
        image_id: tuple(items)
...
def _missing_diagnostics(
    ...
    source_keys = {
        canonical_source_key(path): path.name
        for path in paths
    }
    ...
    debug = "; ".join(
        f"{source_keys[key]} -> {key}"
        for key in candidates
    )

    return (
        f"COCO canonical key: {coco_key}; "
        f"source candidates: {debug or 'none'}"
    )
...
def _match_candidates(
    ...
    """Ищет exact match, затем строгое canonical Roboflow-имя."""
    ...
    exact_relative = tuple(
        ...
        for path in paths
        if (
            path.relative_to(positive_root)
            .as_posix()
            .casefold()
    ...
    coco_basename = Path(normalized_name).name.casefold()

    exact_basename = tuple(
        path
        for path in paths
        if path.name.casefold() == coco_basename
    )
    ...
    return tuple(
        path
        for path in paths
        if canonical_source_key(path) == coco_key
    )
...
def _audit_matches(
    ...
    for image in images:
        ...
        if (
            ...
            raise AuditError(
                "Every COCO image must have integer id "
                "and non-empty file_name"
            )
        ...
        if (
            ...
            raise AuditError(
                f"COCO image {image_id} must have "
        ...
        try:
            ...
            raise AuditError(
                f"Cannot read positive image {path}: {error}"
            ) from error
...
def audit_dataset(
    ...
    duplicate_basenames = sorted(
        name
        for name, count in Counter(basenames).items()
        if count > 1
    )

    if duplicate_names or duplicate_basenames:
        raise AuditError(
            f"Duplicate COCO filenames: {duplicate_names}; "
            f"duplicate basenames: {duplicate_basenames}"
        )
...
def print_audit_report(
    ...
    """Печатает сводку и небезопасные результаты сопоставления."""
    ...
    print(
        ...
        f"{len(audit.images_without_annotations)}"
    )
    ...
    for match in audit.matches.values():
        ...
        if match.status is MatchStatus.MISSING:
            ...
            if missing_details_printed > 5:
                detail = (
                    "Diagnostic details omitted after first "
                    "5 MISSING images"
                )
        ...
        print(
            ...
            f"actual={match.actual_width}x{match.actual_height}; "
            f"{detail}"
        )
...
def _build_rows(
    ...
    for image in sorted(
        ...
        if match.relative_path is None:
            raise AuditError(
                "Successful audit has no path for "
                f"COCO image {image_id}"
            )
        ...
        annotations = audit.annotations_by_image.get(
            image_id,
            (),
        )
...
def _write_dataclasses(
    ...
    """Записывает типизированные строки в CSV."""
...
def _write_negative_images(
    ...
    """Записывает negative-кадры и возвращает счётчики."""
    ...
    for root in negative_roots:
        ...
        if source_set not in allowed_names:
            raise AuditError(
                "Negative directory name must be "
                f"clear_horizon or clear_sky: {root}"
            )

        if source_set in seen_sets:
            raise AuditError(
                f"Duplicate negative source set: {source_set}"
            )
    ...
    if missing_sets:
        raise AuditError(
            f"Missing negative source sets: "
...
def generate_csv(
    ...
    bad_matches = [
        match
        for match in audit.matches.values()
        if match.status is not MatchStatus.MATCH
    ]

    if bad_matches:
        raise AuditError(
            "Ground truth CSV was not generated because "
            "image matching audit failed"
        )

    if audit.images_without_annotations:
        raise AuditError(
            "Ground truth CSV was not generated because "
            "COCO has images without annotation"
        )
    ...
    negative_counts = _write_negative_images(
        output_dir / "negative_images.csv",
        negative_roots,
    )
    ...
    print(
        f"Annotated positive frames: {len(image_rows)}"
    )
    ...
    print(
        "clear_horizon frames: "
        f"{negative_counts['clear_horizon']}"
    )
    print(
        "clear_sky frames: "
        f"{negative_counts['clear_sky']}"
    )
    print(
        "Total negative frames: "
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Audit COCO ground truth and create "
            "real_eval CSV files"
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Negative directory; pass clear_horizon "
            "and clear_sky separately"
        ),
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Output directory for three CSV files",
    )
...
def main(
    ...
    """Проводит аудит и прекращает работу при несоответствии."""
