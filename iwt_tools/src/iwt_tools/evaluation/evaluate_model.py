...
from .evaluation_core import (
    ...
    ObjectMatch,
    PredictedComponent,
    distance_from_centroid_to_bbox,
...
class PositiveObjectRow:
    """Строка отчёта для одного scored ground-truth объекта."""
...
class PositiveImageRow:
    """Строка scored/uncertain диагностики одного positive source-кадра."""
...
def extract_predicted_components(result: PredictionResult) -> tuple[PredictedComponent, ...]:
    ...
    for label in range(1, count):
        ...
        components.append(PredictedComponent(
            index=label, label=label, x1=x, y1=y, x2=x + width, y2=y + height,
            centroid_x=float(centroids[label, 0]), centroid_y=float(centroids[label, 1]),
            area_pixels=area,
            max_score=float(result.probability_map[labels == label].max()),
        ))
...
def _evaluate_positive(
    ...
    for frame in source:
        ...
        frame_matching = match_frame_predictions(ground_truth, predictions)
        ...
        _save_error(
            ...
            _draw_positive_visualization(
                frame.image, result, matches, unmatched, ground_truth.uncertain
            ),
        )
