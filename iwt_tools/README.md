...
статистику рамок. Потенциальный foreground определяется как `mask != 0`, но
объектом становится только 8-связная область, содержащая хотя бы один пиксель
со значением 255. Связанные серые пиксели расширяют рамку подтверждённого
объекта, а отдельные ненулевые области без пикселя 255 учитываются в аудите как
артефакты и не создают bbox. Фильтрация по площади не применяется, поэтому
одиночный пиксель со значением 255 сохраняется как объект.
